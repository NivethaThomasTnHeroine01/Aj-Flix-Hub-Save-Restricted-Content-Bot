    ...
    if FORCE_SUB:
        try:
          ...
          if str(user.status) == "ChatMemberStatus.BANNED":
              await message.reply_text("You are Banned. Contact -- @Ajaytrams")
              return 1
        except UserNotParticipant:
            link = await app.export_chat_invite_link(FORCE_SUB)
            caption = f"Join our channel to use the bot"
            await message.reply_photo(photo="https://graph.org/file/13171fd2ffa55d500976d-442da5fad36533eea5.jpg",caption=caption, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Join Now...", url=f"{link}")]]))
            return 1
...
async def terms(client, message):
    ...
    buttons = InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("💬 Contact Now", url="https://t.me/Ajaytrams")],
        ]
    )
